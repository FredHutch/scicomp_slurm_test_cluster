...
execute 'create slurm user' do
  ...
  action :run
  only_if do
    `/usr/bin/mysql -u root -p#{node['slurm_test_cluster']['mysql']['password']} -D mysql -r -B -N -h 127.0.0.1 -e "SELECT COUNT(*) FROM user WHERE USER = 'slurm' AND HOST = '127.0.0.1'"`.to_i.zero? # ~FC048
  end
end
...
execute 'create slurm account database' do
  ...
  action :run
  not_if do
    `/usr/bin/mysql -u root -p#{node['slurm_test_cluster']['mysql']['password']} -D mysql -r -B -N -h 127.0.0.1 -e "SHOW DATABASES LIKE 'slurm_acct_db'"`.chomp == 'slurm_acct_db' # ~FC048
  end
end
